---
title: "Beyer Orchard Layout (TREE VARIETY)"
output:
  html_document:
    df_print: paged
    code_folding: "hide"
...
trees_final3$Column <- factor(trees_final3$Column, levels = c("A","B","C","D","E","F",
                         ...
                         "M","N","O","P","Q","R","S",
                         "T","U","V","W","X","Y","Z","AA",
                         "BB","CC","DD","EE","FF","GG","HH","II","JJ","KK","LL","MM","NN","OO","PP","ZZZ"))
```
...
ggplot(filter(trees_final3, Row > 58), aes(as.numeric(Column), Row)) + geom_tile(aes(fill = TreeVarietyName),colour = "white",size=2.2) + 
geom_point(data=filter(trees_final3, TreeDeath == 1 & Row > 58), mapping=aes(x=as.numeric(Column), y=Row), shape=4, size=5,alpha=1,stroke=2.5) +
  scale_fill_manual(values=c("purple",#"#542788",
                             #"#d73027",
                             "red",
                             #"#8c510a",
                             #"#fee090",
...
ggplot(filter(trees_final3, Row <= 58), aes(as.numeric(Column), Row)) + geom_tile(aes(fill = TreeVarietyName),colour = "white",size=2.2,stroke=1) + 
geom_point(data=filter(trees_final3, TreeDeath == 1 & Row <= 58), mapping=aes(x=as.numeric(Column), y=Row), shape=4, size=5,alpha=1,stroke=2.5) +
  #scale_fill_discrete() + 
  #scale_fill_brewer(palette = "Set1",type="div") + 
  scale_fill_manual(values=c("purple",#"#542788",
